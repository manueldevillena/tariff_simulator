...
def optimizator(price_photovoltaic, price_battery, inputs, filespath, volume_fee_n, capacity_fee_n, fix_fee_n,
    ...
    def battery_state_of_charge(model, t):
        ...
        """
        if t == 0:
            return model.bat_soc[t] == 0#(model.bat_capacity * 0.5)# - (model.bat_outflow[t] / model.bat_eff_discharge) + (model.bat_inflow[t] * model.bat_eff_charge)
        else:
    ...
    print('Optimisation time is {}'.format(time.time() - time_before))

    #import matplotlib.pyplot as plt

    #imports = [instance.electricity_imports[t].value for t in range(periods-1)]
    #exports = [instance.electricity_exports[t].value for t in range(periods-1)]
    #demand = instance.demand.extract_values()
    #pv = [instance.pv_output[t].value for t in range(periods-1)]

   # demand = to_nparray(demand)

   # fig = plt.figure()
   # plt.plot(imports, linewidth=1.5, color='b')
   # plt.plot(exports, linewidth=1.5, color='r')
    # plt.plot(pv, linewidth=1.5, color='g')
   # plt.plot(demand, linewidth=1.5, color='k')
   # plt.grid()
   # plt.legend(['imports', 'exports', 'demand'])
   # plt.show()
    ...
    import pickle
    with open('./prosumers/profiles.p', 'wb') as f:
        pickle.dump(profiles, f)

    return sizing, economic


def parallel_solve(price_photovoltaic, price_battery, inputs, filespath, volume_fee_n, capacity_fee_n, fixed_fee_n,
                   volume_share, instances):
    print(type(fixed_fee_n))
    pool = Pool(processes=1)
    func = partial(optimizator, price_photovoltaic, price_battery, inputs, filespath, volume_fee_n,
