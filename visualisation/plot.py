...
def plot_line_dynamics(ax, results_mean, results_sem, color_list, column):
    """
...
def plot_line_prices(ax, costs_mean, costs_sem):
    ...
    for key in costs_mean.keys():
        ...
        colors = color_list
        ax.step(x, ((y/y[0])-1)*100, '-o', where='post', color=colors[key], alpha=0.7, label=key)
        ax.fill_between(x, (((y-yerr)/(y-yerr)[0])-1)*100, (((y+yerr)/(y-yerr)[0])-1)*100, step='post', color=colors[key], alpha=0.2)
...
########################################################################################################################
    ...
    plot_line_dynamics(axes[0],dynamics_mean, dynamics_sem, color_list, 'der')
    ...
    fig.savefig('/home/villena/bitbucket/working_paper/IEEEtran/figures/growth_prosumers_prices{}.pdf'.format(which_analysis),
...
########################################################################################################################
    ...
    fig.tight_layout()
    fig.savefig('/home/villena/bitbucket/working_paper/IEEEtran/figures/capacity{}.pdf'.format(which_analysis),
                dpi=fig.dpi, bbox_inches='tight')
########################################################################################################################
